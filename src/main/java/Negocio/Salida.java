...
public class Salida {
    ...
    public String registrarTiquete(Salida salida, String nombre, String documento,
                                Cliente cliente, Puesto myPuesto,
                                double valorPagar, boolean idaYVuelta) {
    int cod = buscarCodUltimoTq();
    Tiquete t = new Tiquete(cod, nombre, documento, cliente, myPuesto, valorPagar, idaYVuelta);
    this.myTiquetes.add(t);
    String tipoViaje = idaYVuelta ? "Ida y Vuelta" : "Solo Ida";
    return "Tiquete vendido con exito:"
            + "\nTiquete: "   + t.getCodTq()
            + "\nPasajero: "  + t.getDocumento() + " --- " + t.getNombre()
            + "\nTelefono: "  + cliente.getTelefono()
            + "\nCorreo: "    + cliente.getCorreo()
            + "\nTipo cliente: " + cliente.getTipoCliente()
            + "\nSalida: "    + salida.getIdSalida()
            + " (" + salida.getRuta().getOrigen() + " ---> " + salida.getRuta().getDestino() + ")"
            + " " + salida.getFecha() + "   " + salida.getHora()
            + "\nBus: "       + salida.getBusAsignado().getPlaca()
            + " (" + salida.getBusAsignado().getTipoServ() + ")"
            + "  Capacidad: " + salida.getBusAsignado().getMyPuestos().length
